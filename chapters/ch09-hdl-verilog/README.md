...
### ตัวอย่าง: ระบบเตือนในรถยนต์ (วงจร PLA จากบทที่ 8 หัวข้อ 8.15.1)
...
### ตัวอย่าง: เครื่องซักผ้าจากบทที่ 8 (หัวข้อ 8.15.3)
...
> การหน่วงเวลาในฮาร์ดแวร์จริงต้องทำด้วย **ตัวนับกับสัญญาณนาฬิกา** เท่านั้น (เหมือนวงจรหน่วงเวลาของแอร์ในบทที่ 8 หัวข้อ 8.15.4)

---

## 9.10 ปลายทางของโค้ด: หนึ่งบรรทัด Verilog กลายเป็นตารางค้นหา

หัวข้อที่แล้วบอกว่าเครื่องมือสังเคราะห์แปลงโค้ดเป็น "เกตและฟลิปฟลอป" — แต่ถ้าเป้าหมายคือ **FPGA** สิ่งที่ได้ไม่ใช่เกตจริง ๆ

### ชิป FPGA ไม่ได้สร้างเกตตามที่เราเขียน

ข้างใน FPGA ไม่มีคลังเกต AND/OR ให้หยิบมาต่อ แต่มี **ตารางค้นหา (Look-Up Table: LUT)** นับแสนตัวกระจายอยู่ทั่วชิป — LUT หนึ่งตัวคือ **หน่วยความจำจิ๋ว** ที่เก็บ "คำตอบ" ของทุกกรณีอินพุตไว้ล่วงหน้า

$$k\text{-input LUT} \;=\; 2^k \times 1 \text{ บิต SRAM} \;+\; \text{มัลติเพล็กเซอร์ } 2^k{:}1$$

- อินพุตของวงจรทำหน้าที่เป็น **สายเลือก** ของมัลติเพล็กเซอร์ (บทที่ 5)
- ค่าที่เก็บไว้ในหน่วยความจำคือ **ตารางความจริง** ของสมการเรา (บทที่ 2–4)
- ป้อนอินพุตเข้ามา มัลติเพล็กเซอร์ก็หยิบบิตที่ตำแหน่งนั้นออกมาทันที

> ⭐ **ผลที่ตามมาที่สำคัญ:** ทุกฟังก์ชันใช้เวลาเท่ากันหมด ไม่ว่าสมการจะง่ายอย่าง $F = A$ หรือซับซ้อนอย่าง $F = A \oplus B \oplus C$ เพราะสัญญาณวิ่งผ่านมัลติเพล็กเซอร์ชั้นเดียวเท่ากัน — การวิเคราะห์ timing จึงง่ายขึ้นมาก

<img src="./images/verilog-to-lut.svg" width="820" alt="สมการ Verilog หนึ่งบรรทัดกลายเป็นค่าบิตใน LUT" style="display:block; margin:1.25rem auto;">

### ให้ simulator บอกค่าใน LUT ให้เราดู

เราไม่ต้องเดาว่าเครื่องมือสังเคราะห์จะหยอดบิตอะไรลง LUT — เขียน testbench ไล่อินพุตครบทุกกรณี เก็บผลลงเวกเตอร์ แล้วพิมพ์ออกมาได้เลย
...
> 🎯 **แปดบิตนี้คือ "วงจร" ทั้งวงจร** — ถ้าเปลี่ยน `10010110` เป็น `11101000` ฮาร์ดแวร์ชุดเดิมจะเปลี่ยนจาก Sum ของ Full Adder เป็น Carry-out ทันที โดยไม่มีสายเส้นไหนขยับเลย
>
> ตอนเรียนบทที่ 8 เราไล่ตารางความจริงแล้วเขียน fuse map ด้วยมือ · ตอนนี้เขียนสมการหนึ่งบรรทัด แล้วเครื่องมือคำนวณบิตให้ครบทุก LUT ในชิป — เมื่อวงจรมี LUT นับแสนตัว นี่คือความต่างระหว่าง "ทำได้" กับ "ทำไม่ได้"

### ภาพรวมของเส้นทางทั้งหมด

$$\underbrace{\texttt{assign f = a \^{} b \^{} c;}}_{\text{บทที่ 9}} \;\longrightarrow\; \underbrace{\text{ตารางความจริง 8 แถว}}_{\text{บทที่ 2--4}} \;\longrightarrow\; \underbrace{\texttt{10010110}}_{\text{บิตใน LUT}} \;\longrightarrow\; \underbrace{\text{SRAM + MUX ในชิป}}_{\text{บทที่ 5--6}}$$
...
| LUT ของ FPGA | 9 | ค่าที่สังเคราะห์ได้จาก `assign` — ดูหัวข้อ 9.10 |
...
### ชุดที่ 3 — เครื่องซักผ้า FSM (เชื่อมกับบทที่ 8 หัวข้อ 8.15.3)
...
### ชุดที่ 4 — อ่านค่าใน LUT ด้วย simulator

วาง `fa_sum` กับ `fa_cout` ไว้ใน **design.sv** และ `tb_lut` ไว้ใน **testbench.sv** ตามโค้ดในหัวข้อ 9.10 — ผลที่ได้คือ `10010110` และ `11101000` ซึ่งคือเนื้อหาทั้งหมดของ LUT สองตัวนั้น
...
แปดบิตนี้คือเนื้อหาของ LUT ที่เครื่องมือสังเคราะห์จะหยอดลงชิป — เปลี่ยนบิต ก็เปลี่ยนวงจร
...
เขียนวงจรควบคุมปั๊มน้ำในบทที่ 8 หัวข้อ 8.15.2 เป็น Verilog
...
ใช้เทคนิคในหัวข้อ 9.10 หาค่า LUT 3 อินพุต (8 บิต) ของฟังก์ชันต่อไปนี้ แล้วตรวจด้วยมือว่าตรงกับตารางความจริง
