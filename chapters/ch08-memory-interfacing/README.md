...
> 💡 มองภาพรวมก่อนเริ่ม: ครึ่งแรกของบทนี้ (8.1–8.11) พูดถึง "หน่วยความจำ" และ "ลอจิกโปรแกรมได้" ส่วนครึ่งหลัง (8.12 เป็นต้นไป) พูดถึงการเชื่อมต่อโลกแอนะล็อกกับโลกดิจิทัลผ่าน ADC/DAC ทั้งสองหัวข้อเชื่อมกันด้วยแนวคิดเดียวกันคือ "การเก็บและถอดรหัสข้อมูลเป็นบิต"
...
#### โครงสร้างภายในของ RAM Array (Gate-Level Implementation)

วงจรจำลองด้านล่างเป็น RAM ขนาด **4 Address × 4 bit** ที่สร้างขึ้นจากลอจิกเกตพื้นฐาน ช่วยให้เห็นว่าคำว่า *Random Access* เกิดขึ้นได้อย่างไรในระดับวงจร

<img src="./images/ram-array-4x4.png" width="650" alt="RAM Array 4 Address x 4 bit ระดับลอจิกเกต">

| ส่วนประกอบในวงจร | หน้าที่ |
|:---|:---|
| **Address Decoder (2→4)** | ถอดรหัส Address 2 บิต ให้เหลือสัญญาณเลือกแถว (Word Line) ที่ทำงานครั้งละ 1 แถวเท่านั้น — นี่คือกลไกที่ทำให้เข้าถึงทุกตำแหน่งได้เร็วเท่ากัน |
| **Write / Read** | สัญญาณควบคุมทิศทางข้อมูล ทำงานร่วมกับ Word Line เพื่อกำหนดว่าแถวที่ถูกเลือกจะ *รับ* ข้อมูลจาก In0–In3 หรือ *ส่ง* ข้อมูลออกที่ O0–O3 |
| **Clk** | จังหวะสัญญาณนาฬิกาที่บันทึกข้อมูลลง Storage Cell (แต่ละเซลล์คือ Flip-Flop 1 บิต ซึ่งตรงกับคอลัมน์ SRAM ในตารางข้างต้น) |
| **In0–In3 / O0–O3** | Data Bus ขาเข้าและขาออก กว้าง 4 บิต เท่ากับความกว้างข้อมูล 1 ตำแหน่ง |
| **Multiplexer แถวล่าง** | เลือกว่าจะให้เซลล์ของแถวใดเป็นตัวขับ Data Out ตาม Address ที่ป้อนเข้ามา |

> 💡 ความจุของวงจรนี้ = $2^2$ Address × 4 bit = **16 บิต (2 ไบต์)** ตามสูตรในหัวข้อ 8.2 — สังเกตว่าถ้าเพิ่มเส้น Address เพียง 1 เส้น จำนวนแถวและจำนวนเซลล์จะเพิ่มเป็น 2 เท่าทันที

#### มุมมองแบบไอซี (Black Box View)

ในการออกแบบระบบจริงเราไม่ได้ต่อเกตทีละตัว แต่เรียกใช้ชิป RAM สำเร็จรูป ตัวอย่างคือ RAM ขนาด 16 ไบต์ (16 Address × 8 bit)

<img src="./images/ram-ic-16b.png" width="650" alt="RAM IC ขนาด 16 ไบต์ แบบ Black Box">

| ขา | ความหมาย |
|:---|:---|
| **A** | Address Bus 4 บิต → เข้าถึงได้ $2^4 = 16$ ตำแหน่ง |
| **DI** (Data In) | ข้อมูลขาเข้า 8 บิต |
| **W** (Write Enable) | เลือกโหมดทำงาน: เขียน (W = 1) หรืออ่าน (W = 0) |
| **DO** (Data Out) | ข้อมูลขาออก 8 บิต |
...
#### โครงสร้างภายในของ ROM: Decoder + OR Array

ROM ไม่ได้ "จำ" ข้อมูลด้วย Flip-Flop เหมือน RAM แต่ **เดินสายกำหนดค่าไว้ล่วงหน้า** ว่าแต่ละ Address จะให้ข้อมูลอะไร โครงสร้างจึงมี 2 ส่วนคือ Address Decoder และ OR Array

<img src="./images/rom-4addr-4bit.png" width="650" alt="ROM 4 Address x 4 bit สร้างจาก Decoder และ OR Array">

**ROM 4 Address × 4 bit** — Decoder 2→4 เลือก Word Line ทีละเส้น จุดที่ Word Line ต่อเข้ากับ OR Gate ของคอลัมน์ใดจะให้บิต `1` ของคำนั้น (จุดที่ไม่ต่อคือ `0`) ตารางด้านขวาในรูปคือข้อมูลที่ถูกเดินสายไว้

| Address | ค่าที่เก็บ (Hex) | O3 O2 O1 O0 |
|:---:|:---:|:---:|
| 00 | B | 1011 |
| 01 | 3 | 0011 |
| 10 | D | 1101 |
...
ในรูปป้อน Address = `10` เอาต์พุตจึงเป็น `1101` = D ตรงตามตาราง

<img src="./images/rom-8addr-5bit.png" width="650" alt="ROM 8 Address x 5 bit สร้างจาก Decoder และ OR Array">

**ROM 8 Address × 5 bit** — ขยายเป็น Decoder 3→8 และเพิ่มเป็น 5 คอลัมน์ โดย O4 ถึง O0 มีน้ำหนักประจำหลัก 16, 8, 4, 2, 1 ตามลำดับ ในรูปป้อน Address = `111` เอาต์พุตจึงเป็น `01111` = 15 (F)

> 💡 โครงสร้าง **Decoder + OR Array** นี้มีรากเดียวกับ PLA และ PROM ในหัวข้อ 8.7 ต่างกันที่ ROM ใช้ AND Array แบบตายตัว (ถอดรหัสครบทุก Minterm) ส่วน PLA โปรแกรมได้ทั้ง AND Array และ OR Array
...
### 3. การขยายหน่วยความจำด้วยการถอดรหัสที่อยู่ (Memory Expansion)

เมื่อชิปตัวเดียวมีความจุไม่พอ เราจะต่อชิปหลายตัวขนานกันบน Data Bus แล้วใช้บิต Address ที่สูงกว่ามาเลือกว่าจะให้ชิปตัวใดตอบสนอง ตัวอย่างต่อไปนี้ขยายจาก ROM 16 × 8 หนึ่งตัวไปเป็น 64 × 8

<img src="./images/rom-16addr-8bit.png" width="650" alt="ROM 16 Address x 8 bit ชิปเดียว">

**จุดเริ่มต้น: ROM 16 × 8 หนึ่งตัว** — Address กว้าง 4 บิต ($2^4 = 16$ ตำแหน่ง) ข้อมูลออก 8 บิต ขา `En` คือสัญญาณ Enable / Chip Select ในรูปป้อน Address = `0010` จึงอ่านค่าที่ตำแหน่งที่ 2 ได้เป็น `0D`h = `00001101`

<img src="./images/rom-32addr-8bit.png" width="650" alt="ROM 32 Address x 8 bit จากชิป 16x8 สองตัว">

**ขยายเป็น 32 × 8 ด้วยชิป 2 ตัว** — Address กว้างขึ้นเป็น 5 บิต โดย A3–A0 ป้อนขนานให้ทั้งสองชิปเหมือนกัน และใช้ **A4 (บิตสูงสุด) เลือกชิป**

| A4 | ชิปที่ถูกเลือก | ช่วงที่อยู่ (Hex) |
|:---:|:---:|:---:|
| 0 | ชิป 0 | 00h – 0Fh |
| 1 | ชิป 1 | 10h – 1Fh |

<img src="./images/rom-64addr-8bit.png" width="650" alt="ROM 64 Address x 8 bit จากชิป 16x8 สี่ตัว">

**ขยายเป็น 64 × 8 ด้วยชิป 4 ตัว** — Address กว้าง 6 บิต โดย A3–A0 ยังป้อนขนานเหมือนเดิม แต่ใช้ **A5 A4 จำนวน 2 บิต เลือก 1 ใน 4 ชิป**

| A5 A4 | ชิปที่ถูกเลือก | ช่วงที่อยู่ (Hex) |
|:---:|:---:|:---:|
| 00 | ชิป 0 | 00h – 0Fh |
| 01 | ชิป 1 | 10h – 1Fh |
| 10 | ชิป 2 | 20h – 2Fh |
...
> 💡 สังเกตรูปแบบที่เกิดซ้ำทุกครั้ง: **บิต Address ล่างเลือกตำแหน่งภายในชิป ส่วนบิต Address บนเลือกตัวชิป** ทุกครั้งที่เพิ่มบิตเลือกชิป 1 บิต จำนวนชิปที่รองรับจะเพิ่มเป็น 2 เท่า วงจรจำลองนี้ใช้ Multiplexer รวมเอาต์พุตเพื่อให้เห็นภาพง่าย แต่ในทางปฏิบัติจะใช้ Decoder 74138 สร้างสัญญาณ CS ให้ชิปที่ไม่ถูกเลือกอยู่ในสถานะ High-Z แล้วต่อ Data Bus ร่วมกันแทน
...
> 💡 ตัวอย่างลิฟต์นี้คือเหตุผลที่ใช้ **PLA** แทน **PAL** — เพราะ OR Array ของ PLA โปรแกรมได้ จึงสามารถนำ Product Term เดียวกัน (เช่นเงื่อนไขความปลอดภัย) ไป "แชร์" ให้กับเอาต์พุตหลายตัว (Brake, Open, Alarm) ได้พร้อมกัน ลดความซับซ้อนของวงจรลงมาก เทียบกับ PAL ที่ต้องสร้าง Product Term ซ้ำทุกครั้งตามที่เห็นในหัวข้อ 8.8
...
## 8.13 โปรโตคอลการสื่อสารดิจิทัลพื้นฐาน (Serial Protocols)
...
## 8.16 DAC (Digital-to-Analog Converter)
...
## 8.17 ADC (Analog-to-Digital Converter)
...
## 8.19 Tinkercad: สร้างวงจรดิจิทัลจำลอง
...
## 8.21 ตัวอย่างการนำไปใช้งานจริง (Real-world Interfacing Applications)

### 8.21.1 ระบบวัดอุณหภูมิดิจิทัล (Digital Temperature Monitor)
...
### 8.21.2 ระบบควบคุมความเร็วมอเตอร์ (PWM Speed Control)
...
> 💡 ทั้งสองระบบในหัวข้อนี้ใช้สถาปัตยกรรมแบบเดียวกันคือ **เซนเซอร์/ตรรกะ → วงจรแยกสัญญาณ (Isolation) → วงจรขับกำลัง (Driver) → อุปกรณ์จริง (Actuator)** ลองสังเกตว่าหลักการ "แยกสัญญาณก่อนขับกำลังสูง" นี้ซ้ำกับระบบลิฟต์ในหัวข้อ 8.10 ด้วย — เป็นรูปแบบมาตรฐานของการอินเตอร์เฟซวงจรดิจิทัลกับโลกจริง
