...
### 2. สมการตรรกะระดับสูง (Full Descriptive Boolean Logic)

เพื่อให้มอเตอร์ทำงานได้อย่างปลอดภัย เราจะใช้เงื่อนไข **Safety_Ready ($S$)**:
$S = \text{Status_Door_Closed} \cdot \overline{\text{Safety_Overweight}} \cdot \overline{\text{Safety_Emergency}} \cdot \overline{\text{Safety_Fire_Alarm}} \cdot \overline{\text{Safety_Motor_Hot}}$

**สมการเอาต์พุต (SOP):**
- **Motor_Move_Up ($O_0$):**
  $O_0 = [(\text{Sensor_Floor_1} \cdot (\text{Button_Call_2+3+4})) + (\text{Sensor_Floor_2} \cdot (\text{Button_Call_3+4})) + (\text{Sensor_Floor_3} \cdot \text{Button_Call_4})] \cdot S$
- **Motor_Move_Down ($O_1$):**
  $O_1 = [(\text{Sensor_Floor_4} \cdot (\text{Button_Call_3+2+1})) + (\text{Sensor_Floor_3} \cdot (\text{Button_Call_2+1})) + (\text{Sensor_Floor_2} \cdot \text{Button_Call_1})] \cdot S$
- **Motor_Speed_High ($O_2$):** 
  $O_2 = [(\text{Sensor_Floor_1} \cdot (\text{Button_Call_3+4})) + (\text{Sensor_Floor_4} \cdot (\text{Button_Call_2+1}))] \cdot S$
- **Door_Open_Command ($O_4$):**
  $O_4 = (\text{Sensor_Floor_1} \cdot \text{Button_Call_1}) + (\text{Sensor_Floor_2} \cdot \text{Button_Call_2}) + (\text{Sensor_Floor_3} \cdot \text{Button_Call_3}) + (\text{Sensor_Floor_4} \cdot \text{Button_Call_4}) + \text{Sensor_Obstruction} + \text{Safety_Overweight}$
- **System_Alarm_Siren ($O_5$):**
  $O_5 = \text{Safety_Overweight} + \text{Safety_Emergency} + \text{Safety_Fire_Alarm} + \text{Safety_Motor_Hot}$
